...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CAPACITY-002 safe cache-only capacity plan.")
...
def main() -> int:
    ...
    if any((args.ibkr, args.orders, args.signals, args.preview, args.approve_candidates)):
        print("CAPACITY-002 refuses IBKR, orders, signals, preview, and candidate approval.", file=sys.stderr)
        return 2
    ...
    try:
        payload = run_capacity_plan(config)

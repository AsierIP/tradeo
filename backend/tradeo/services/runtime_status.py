...
def write_entry_scan_status(module: str, result: dict[str, Any], settings: Settings | None = None) -> None:
    ...
    zero_order_scan = execute_orders and matches_found > 0 and orders_submitted == 0 and not skipped_reason
    zero_order_scan_streak = (
        int(previous.get("zero_order_scan_streak") or 0) + 1
        if zero_order_scan
        else 0
    )
    payload[module] = {
        ...
        "zero_order_block_reason": _entry_scan_zero_order_reason(result) if zero_order_scan else None,

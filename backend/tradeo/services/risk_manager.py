...
class RiskManager:
    ...
    def validate_candidate(
        ...
        *,
        execution_context: Literal["laboratory", "live"] = "live",
    ) -> RiskDecision:
        hard: list[str] = []
        warnings: list[str] = []
        settings = self.settings
        laboratory_context = execution_context == "laboratory"
        ...
        if not laboratory_context and candidate.side == "long" and not settings.allow_longs:
            hard.append("longs_disabled")
        if not laboratory_context and candidate.side == "short" and not settings.allow_shorts:
            hard.append("shorts_disabled")
        if laboratory_context and candidate.reward_risk < settings.min_reward_risk:
            warnings.append(f"reward_risk_below_{settings.min_reward_risk}")
        elif candidate.reward_risk < settings.min_reward_risk:
            hard.append(f"reward_risk_below_{settings.min_reward_risk}")
        if not laboratory_context and open_positions >= settings.max_open_positions:
            hard.append("max_open_positions_reached")
        if (
            not laboratory_context
            and realized_today <= -settings.initial_capital_usd * settings.daily_loss_limit_pct
        ):
            hard.append("daily_loss_limit_reached")
        if not laboratory_context and db is not None:
        ...
        if qty <= 0:
            if laboratory_context and candidate.entry > 0:
                qty = 1
                risk_usd = round(risk_per_share, 2)
                notional = round(candidate.entry, 2)
                warnings.append("laboratory_minimum_qty_1")
            else:
                hard.append("position_size_zero")
        if (
            not laboratory_context
            and notional > equity * settings.max_gross_exposure_pct
            and not settings.allow_margin
        ):
            hard.append("gross_exposure_exceeds_cash_limit")
        if not laboratory_context and risk_usd > risk_budget * 1.0001:
        ...
        if (
            laboratory_context
            and candidate.features.get("avg_dollar_volume", 0) < settings.min_avg_dollar_volume
        ):
            warnings.append("liquidity_filter_failed")
        elif candidate.features.get("avg_dollar_volume", 0) < settings.min_avg_dollar_volume:
            hard.append("liquidity_filter_failed")
        if laboratory_context and candidate.features.get("atr_pct", 0) > settings.max_atr_pct:
            warnings.append("atr_filter_failed")
        elif candidate.features.get("atr_pct", 0) > settings.max_atr_pct:

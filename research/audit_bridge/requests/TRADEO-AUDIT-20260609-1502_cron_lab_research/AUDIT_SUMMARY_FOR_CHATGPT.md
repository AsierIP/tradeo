...
**Validation stance:** no pattern is approved automatically. This package includes 0 paper trades and 2 reconstructed fills; Director must still verify execution quality before promotion.
...
| risk | count | notes |
|---|---:|---|
| possible duplicated event rows | 155 | Counted from repeated `duplicate_group_id`. |
| signals not marked independent | 190 | Any `is_independent_sample` value other than `true`. |
...
| likely_duplicates | 155 | Deduplicate or explain before ranking. |

...
| risk | count | notes |
|---|---:|---|
| possible duplicated event rows | 55 | Counted from repeated `duplicate_group_id`. |
| signals not marked independent | 102 | Any `is_independent_sample` value other than `true`. |
...
| likely_duplicates | 55 | Deduplicate or explain before ranking. |
